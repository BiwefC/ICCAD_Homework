...
class Component(object):
    ...
    def matrixFill(self, G, B, vectorI, vectorE):
        ...
        if self.name.startswith(('R', 'Z')): # Resistor handling
...
def MNACompute(netlistFile, v = True):
    with open(netlistFile) as netlist:
        ...
        for comp in circuit:
            if comp.name.startswith(('R', 'V', 'I', 'Z')):
        ...
        B = np.zeros([N, M], dtype = np.complex)
        vectorI = np.zeros([N, 1], dtype = np.complex)
        vectorE = np.zeros([M, 1], dtype = np.complex)
        ...
        if v:
            print("=" * 10 + " Sorted by Name " + "=" * 10)
            for node in sorted(nodeDict.keys()):
                result = Result[nodeDict[node]]
                if result.imag == 0:
                    print("node %s: %.6fV" % (node, result.real))
                else:
                    # for complex computing.
                    print("node %s: %rV" % (node, result[0]))

            if usingMNA:
                for vsrc in sorted(vsrcDict.keys()):
                    result = Result[vsrcDict[vsrc] + N]
                    if result.imag == 0:
                        print("vsource %s: %.6fA" % (vsrc, result.real))
                    else:
                        # for complex computing.
                        print("vsource %s: %rA" % (vsrc, result[0]))

            print("=" * 10 + " Sorted by Value " + "=" * 10)
            for node in sorted(nodeDict.keys(), key = lambda node: abs(Result[nodeDict[node]])):
                result = Result[nodeDict[node]]
                if result.imag == 0:
                    print("node %s: %.6fV" % (node, result.real))
                else:
            ...
            if usingMNA:
                for vsrc in sorted(vsrcDict, key = lambda node: abs(Result[vsrcDict[node] + N])):
                    result = Result[vsrcDict[vsrc] + N]
                    if result.imag == 0:
                        print("vsource %s: %.6fA" % (vsrc, result.real))
                    else:
